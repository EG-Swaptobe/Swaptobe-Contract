...
// This is used to deploy 2 TBRC20 tokens under names TokenC and TokenD
const deployTokens: DeployFunction = async function(
  ...
  log("----------------------------------------------------");
  log("Deploying 2 TBRC20 tokens and waiting for confirmations...");
  ...
  if (!developmentChains.includes(network.name)) {
    ...
    await verify(tokenC.address, [expandTo18Decimals(10000n)], "contracts/core/test/TBRC20.sol:TBRC20");
    await verify(tokenD.address, [expandTo18Decimals(10000n)], "contracts/core/test/TBRC20.sol:TBRC20");
  }
};
